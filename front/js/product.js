...
.then(product => {                                                /*product contient le kanapés*/
  let test = document.querySelector('.item__img');               /*création du dom*/   /*Image*/
  let image = document.createElement('img');
  image.setAttribute(`src`, product.imageUrl);
  image.setAttribute(`alt`, `Photographie d'un canapé`);
  test.appendChild(image);                                                 /*injectent la variable image  en tant que fils de la div test*/
  ...
  classtitre.appendChild(prix);                                  /*injectent la variable prix  en tant que fils de la div classtitre(titre prix)*/ 
...
let aver = document.getElementById("addToCart")                      /*récupération de la div du bouton ajouter au panier"*/
  aver.addEventListener("click",() =>{                               /*j'écoute le click sur le bouton précédemment récupérer*/
    ...
    if (panier){                                                 /*1 cas*/ /* si panier existe */  /*alors*/
      ...
      if(position >= 0){ /*je teste si position est >= a 0 fin d'éviter le -1 du tableau */   /*alors*/
      ...
      }else{ /*2cas*//*si dans le panier il ni y a pas de id identique et couleur indentique alors on push new product dans la variable panier et on l'envoie au local storage*/
        panier.push(newProduct);  
        localStorage.setItem("productos", JSON.stringify(panier));
      }
    }else{                                                                      /*3cas*/ /*si le panier n'existe pas alors je le créé new panier */
    ...
    window.location.href="./cart.html"                                          /*est nous passons  à la page panier*/
  })
